...
func _ready():
	...
	caterpillar = classLoad.new()


func draw_caterpillar():
	print("draw")
	if(caterpillar.body.size() > $caterpillar.get_child_count()):
		var lastChilde = $sncaterpillarake.get_child($caterpillar.get_child_count()-1).duplicate()
...
func _input(_event):
	if Input.is_action_pressed("ui_right") and not caterpillar.direction == Vector2(-caterpillar.width,0):
		caterpillar.direction = Vector2(caterpillar.width,0)
		...
		caterpillar.direction = Vector2(0,caterpillar.width)
